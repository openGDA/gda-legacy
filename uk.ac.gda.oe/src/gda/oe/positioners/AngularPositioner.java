...
/**
 * A class for positioning a motor in terms of angular position.
 */
public class AngularPositioner extends Positioner {
	private static final Logger logger = LoggerFactory.getLogger(AngularPositioner.class);
	...
	/**
	 * Moves the positioner to the specified position. This method converts motor position from GDA value in drive Unit
	 * to EPICS value in .EGU unit.
	 * 
	 * @param position
	...
	/**
	 * update the current motor position in GDA. This method converts EPICS motor position to GDA motor position in
	 * Drive Unit.
	 * 
	 * @param motorPosition
	 */
	...
	/**
	 * sets the softlimits for the motor. This method takes limit values in GDA drive unit and converts them to EPICS
	 * value in EGU unit before call the actual motor.
	 * 
	 * @param lowLimit
	 * @param highLimit
	 * @throws MotorException
	...
	protected boolean checkIncrement(Quantity increment) {
		...
		// stepsPerMdeg is what this positioner knows
		try {
			targetPosition = currentPosition + ((Angle) increment).to(driveUnit).getAmount();
			rtrn = true;
		} catch (ConversionException cce) {
			// Deliberately do nothing because if the cast fails then the
			// targetQuantity is not a angle so we should return false
		}
		return rtrn;
	}
	...
	public void setSpeed(Quantity speed) throws MoveableException {
		// Unit u = speed.getUnit();
		/*
		 * if (!u.equals(QuantityFactory.createUnitFromString("mDeg/s")) &&
		 * !u.equals(QuantityFactory.createUnitFromString("Hz mDeg"))) { throw new MoveableException(new MoveableStatus(
		 * MoveableStatus.SPEEDLEVEL_ERROR, getName()), "Incorrect type of speed for AngularPositioner: " + speed); }
		 */
