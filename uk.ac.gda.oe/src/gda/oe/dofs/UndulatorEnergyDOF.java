...
/**
 * A DOF responsible for setting the energy of an undulator. Needs to know about the Gap, LowerPhase and UpperPhase
 * Positioners. This DOF cannot determine its position from the positions of its Moveables (the Gap, LowerPhase and
 * UpperPhase DOFs of the Undulator). It must assume that its position changes as the result of a successful move. In
 * order to determine that this has happenned it must override some methods of the DOF class in order to provide itself
 * with further information.
 */

public class UndulatorEnergyDOF extends DOF implements MultipleMoveWatcher, UndulatorMoveCalculatorWatcher {
	private static final Logger logger = LoggerFactory.getLogger(UndulatorEnergyDOF.class);
	 ...
	 * Constructor
	 */
	public UndulatorEnergyDOF() {
		dofType = DOFType.UndulatorEnergyDOF;
	}

	/**
	 * This exists so that UndulatorEnergyDOF can be a Moveable but it never actually gets called because the
	...
	protected Quantity checkTarget(Quantity newQuantity) {
		...
		try {
			...
			// targetQuantity is not an Energy so we should return null
		}
		return rtrn;
	}
	...
	public void configure() throws FactoryException {
		...
		// The move calculator guesses a starting energy so get it
		// to use here as well.
		setCurrentQuantity(umm.getEnergy());
		...
		// This is needed to get things into the right starting state
		updateStatus();
	}
	...
	public DOFCommand createAbsoluteMover(Quantity position) throws MoveableException {
		...
		if (newPosition != null) {
			umm.setRequestedEnergy((Energy) position);
	...
	protected void setValidAcceptableUnits() {
		...
		validAcceptableUnits.add(NonSI.ELECTRON_VOLT);
		validAcceptableUnits.add(SI.KILO(NonSI.ELECTRON_VOLT));
	}
	...
	public boolean unLock(Object unLocker) {
		logger.debug("UndulatorEnergyDOF being unlocked by " + unLocker);
		...
		if (moveStarted) {
			...
			if (statusCode == MoveableStatus.READY) {
				if (moveFailed) {
					logger.debug("UndulatorEnergyDOF unsuccessful move. Message was:" + moveFailureMessage);
					setPositionValid(false);
					notifyIObservers(this, new MoveableException(new MoveableStatus(statusCode, getName()),
							"UndulatorEnergyDOF unsuccessful move. Message was:\n" + moveFailureMessage));
					moveFailed = false;
				} else {
					umm.moveDone();
				}
