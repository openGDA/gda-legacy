...
/**
 * A DOF responsible for setting the harmonic of an undulator. Needs to know about the Gap, LowerPhase and UpperPhase
...
public class UndulatorHarmonicDOF extends DOF implements MultipleMoveWatcher, UndulatorMoveCalculatorWatcher {
	private static final Logger logger = LoggerFactory.getLogger(UndulatorHarmonicDOF.class);
	 ...
	 * Constructor
	 */
	public UndulatorHarmonicDOF() {
		dofType = DOFType.HarmonicDOF;
	}
	 ...
	 * @return a Dimensionless of the same numerical value and Units
	 */
	@Override
	protected Quantity checkTarget(Quantity newQuantity) {
		Quantity rtrn = null;
		if (newQuantity.getUnit() == Unit.ONE) {
			rtrn = newQuantity;
		}
		return rtrn;
	}
	...
	public void configure() throws FactoryException {
		...
		// The move calculator guesses a starting Harmonic so get it
		// to use here as well.
		setCurrentQuantity(umm.getHarmonic());
	...
	public DOFCommand createAbsoluteMover(Quantity position) throws MoveableException {
		...
		if (newPosition != null) {
			umm.setRequestedHarmonic(position);
	...
	public void inform(Quantity newEnergy, Quantity newHarmonic, Quantity newPolarization, boolean valid) {
		setCurrentQuantity(newHarmonic);
	...
	protected void setDefaultAcceptableUnits() {
		...
		defaultAcceptableUnits.add(Unit.ONE);
	}
	...
	protected void setValidAcceptableUnits() {
		...
		validAcceptableUnits.add(Unit.ONE);
	}
	...
	public boolean unLock(Object unLocker) {
		logger.debug("UndulatorHarmonicDOF being unlocked by " + unLocker);
