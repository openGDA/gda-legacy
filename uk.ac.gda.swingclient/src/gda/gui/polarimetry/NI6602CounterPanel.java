...
/**
 * Provides basic user panel to drive up to 2 National Instruments PCI 6602 counters
 */
public class NI6602CounterPanel extends AcquisitionPanel implements Runnable {
	private Thread runner;
	private boolean running = false;

	private static Detector counter1 = null;
	private static Detector counter2 = null;
		...
		while (running) {
			...
			if (busy != Detector.BUSY && finished) {
				if (startCounters()) {
					...
					// jCheckCounting.setSelected(true);
					finished = false;
				} else {
			...
			if (busy == Detector.BUSY) {
				...
				// jCheckCounting.setSelected(false);
	...
	private JPanel getJButtonPanel() {
		...
		// jButtonPanel.add(getJCheckCounting());
	...
	private JButton getJButtonStart() {
		...
		jButtonStart.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				runner = uk.ac.gda.util.ThreadManager.getThread(NI6602CounterPanel.this, getClass().getName());
				running = true;
				runner.start();
			}
		});
	...
	private JPanel getJDataPanel() {
		// jLabelChannel = new JLabel[totalChannels];
	...
	/*
	 * private JCheckBox getJCheckCounting() { jCheckCounting = new JCheckBox(); jCheckCounting.setText("Counting");
	 * jCheckCounting.setHorizontalAlignment(SwingConstants.RIGHT);
	 * jCheckCounting.setHorizontalTextPosition(SwingConstants.LEADING);
	 * jCheckCounting.setVerticalAlignment(SwingConstants.CENTER); return jCheckCounting; }
	 */
