 ...
 */
public class JythonController extends JPanel implements IObserver {
	 ...
	 */
	public static final String NAME = "JythonController";
	...
	public void configure() {
		if (!configured) {
			...
			if (!(stateDisplayPanels.isEmpty())) {
				for (StateDisplayPanel panel : stateDisplayPanels) {
					this.add(panel);
				}
			}
	...
	private void jbInit() {
		...
		btnPanic.setForeground(Color.black);
		btnPanic.setText("STOP ALL");
		btnPanic.addActionListener(new JythonTerminal_btnPanic_actionAdapter(this));
		...
		btnPanic.setAlignmentX(Component.CENTER_ALIGNMENT);
		this.add(btnPanic);
		
	}

class JythonTerminal_btnPauseScan_actionAdapter implements java.awt.event.ActionListener {
	JythonController adaptee;

	JythonTerminal_btnPauseScan_actionAdapter(JythonController adaptee) {
...
class JythonTerminal_btnPauseScript_actionAdapter implements java.awt.event.ActionListener {
	...
	JythonTerminal_btnPauseScript_actionAdapter(JythonController adaptee) {
		this.adaptee = adaptee;
	}
...
class JythonTerminal_btnPanic_actionAdapter implements java.awt.event.ActionListener {
	...
	JythonTerminal_btnPanic_actionAdapter(JythonController adaptee) {
		this.adaptee = adaptee;
	}
...
class JythonTerminal_btnHaltScriptScan_actionAdapter implements ActionListener {
	private JythonController adaptee;

	JythonTerminal_btnHaltScriptScan_actionAdapter(JythonController adaptee) {
		this.adaptee = adaptee;
	}
